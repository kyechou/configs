...
mkdir -p logs
touch logs/{stats,public,private,trade}
...
if ! tmux has-session -t batmud 2>/dev/null; then
    ...
    tmux split-window -t batmud:batmud.0 -bv -l 15 'watch -ctn 0.5 "cat logs/stats"'
    tmux split-window -t batmud:batmud.1 -v -l  15 'tail -f logs/private'
    tmux split-window -t batmud:batmud.4 -v -l  15 'tail -f logs/trade'
    tmux select-pane -t batmud:batmud.3
fi
